...
import { GmailProvider, CalendarProvider, MessagingProvider } from "@brain-box-ai/integrations";
import { getLLMProvider } from "@brain-box-ai/ai";
...
export function interpolate(templateStr: string, context: Record<string, any>): string {
...
export function resolveParams(data: Record<string, any>, context: Record<string, any>): Record<string, any> {
...
export class WorkflowExecutor {
  ...
  async executeWorkflow(nodes: WorkflowNode[], edges: WorkflowEdge[], triggerNodeId: string, triggerData?: any) {
    this.nodes = nodes;
    this.edges = edges;
    ...
    try {
      ...
      const { data: activeExecution } = await supabaseAdmin.from("Execution")
      ...
      const isWaiting = stepsList.some((s: any) => s.status === "WAITING");
   ...
   */
  async resumeWorkflow(nodes: WorkflowNode[], edges: WorkflowEdge[], resumedNodeId: string, inputData: any) {
    this.nodes = nodes;
    this.edges = edges;
    
    // Load previously recorded step outputs to reconstruct evaluation context
    const { data: previousSteps } = await supabaseAdmin.from("ExecutionStep")
      .select("*")
    ...
    try {
      ...
      const isWaiting = activeStepsList.some((s: any) => s.status === "WAITING");
  ...
  private getNextNodes(currentNodeId: string): WorkflowNode[] {
  ...
  private async executeNode(node: WorkflowNode) {
    // Record step started
    const { data: step, error: stepError } = await supabaseAdmin.from("ExecutionStep")
      .insert({
    ...
    try {
      ...
      await supabaseAdmin.from("ExecutionStep")
        .update({ input: params })
        .eq("id", step.id);
      ...
      switch (node.type) {
        ...
        case "action.gmail.search": {
          const provider = new GmailProvider(this.workspaceId);
        ...
        case "action.calendar.list": {
          const provider = new CalendarProvider(this.workspaceId);
          output = await provider.list(params.timeRange || "today");
          break;
        }
        case "action.calendar.create": {
          const provider = new CalendarProvider(this.workspaceId);
          output = await provider.create(
            params.title,
            params.startTime,
        ...
        case "action.telegram.send": {
          output = await MessagingProvider.sendTelegram(params.message);
          break;
        }
        case "action.discord.send": {
          output = await MessagingProvider.sendDiscord(params.message, params.webhookUrl);
          break;
        }
        case "action.slack.send": {
          output = await MessagingProvider.sendSlack(params.channel || "general", params.message);
          break;
        }
        ...
        case "action.approval": {
          ...
          const { data: execData } = await supabaseAdmin.from("Execution")
            .select("workflowId")
            .eq("id", this.executionId)
            .single();

          // Create approval log
          await supabaseAdmin.from("Approval").insert({
        ...
        case "logic.loop": {
          ...
          if (Array.isArray(loopItems)) {
            ...
            for (const item of loopItems) {
              const itemContext: Record<string, any> = { item };
