...
async function seed() {
  ...
  const { data: publicUser, error: publicUserError } = await supabaseAdmin.from("User")
  ...
  const { data: existingWu, error: wuCheckError } = await supabaseAdmin.from("WorkspaceUser")
    .select("workspaceId")
    .eq("userId", user.id)
    .limit(1)
    .maybeSingle();
  ...
  if (!workspaceId) {
    ...
    const { data: ws, error: wsError } = await supabaseAdmin.from("Workspace")
    ...
    const { error: prefError } = await supabaseAdmin.from("UserPreference")
      .insert({
        workspaceId,
        theme: "system",
  ...
  // 4. Seed Templates
  const { data: existingTemplates, error: tCheckError } = await supabaseAdmin.from("Template").select("id, title");
  if (tCheckError) {
  ...
  if (!existingTemplates || existingTemplates.length === 0) {
    ...
    const { error: tError } = await supabaseAdmin.from("Template").insert(templates);
    if (tError) {
