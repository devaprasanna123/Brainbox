    ...
    try {
        ...
        if (useSSE) {
            ...
            if (executedToolResults.length > 0) {
                ...
                const finalResponse = await provider.streamChat(toolResultContext, (chunk) => {
                    if (!res.writableEnded) {
            ...
            (async () => {
                if (recentMessages.length <= 3) {
                    try {
                        ...
                        if (title) {
                            await database_1.supabaseAdmin.from("Conversation").update({ title, updatedAt: new Date().toISOString() }).eq("id", conversationId);
                        }
                    }
                    catch (e) { }
                }
