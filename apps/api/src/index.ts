...
import { supabaseAdmin, createServerClient } from "@brain-box-ai/database";
import { encryptCredential, decryptCredential, verifyFirebaseIdToken } from "@brain-box-ai/auth";
import { getLLMProvider, isWakePhrase, retrieveRelevantMemories, MemoryRecord } from "@brain-box-ai/ai";
import { getVoiceProvider } from "@brain-box-ai/voice";
import { getGoogleOAuth2Client, generateOAuthState, consumeOAuthState, verifyWebhookSignature, google, validateOAuthConfiguration, validateGoogleConnection, mapGmailError, REQUIRED_GOOGLE_SCOPES, safeLog } from "@brain-box-ai/integrations";
import { WorkerQueue, calculateNextRun } from "@brain-box-ai/worker";
...
export async function authenticateToken(
  ...
  try {
    ...
    let { data: wu, error: wuError } = await supabaseAdmin
      .from("WorkspaceUser")
      .select("workspaceId")
      .eq("userId", uid)
      .limit(1)
    ...
    if (wuError || !wu) {
      ...
      const { error: wuLinkError } = await supabaseAdmin.from("WorkspaceUser").insert({
      ...
      await supabaseAdmin.from("UserPreference").upsert(
        {
          workspaceId: workspace.id,
          theme: "system",
...
app.patch("/api/conversations/:id", authenticateToken, async (req: AuthenticatedRequest, res) => {
...
const processedRequestIds = new Map<string, { timestamp: number; response?: any }>();
const REQUEST_ID_TTL_MS = 5 * 60 * 1000;

function isDuplicateRequest(clientRequestId?: string): boolean {
...
app.post("/api/chat", authenticateToken, async (req: AuthenticatedRequest, res) => {
  ...
  try {
    ...
    const userMsgRes = await supabaseAdmin
      .from("Message")
      .insert({ conversationId, role: "USER", content, type: "TEXT" })
      .select()
      .single();

    if (userMsgRes.error) throw userMsgRes.error;
    ...
    const [messagesListRes, summaryRes, allMemoriesRes, prefRes, userProfileRes] = await Promise.all([
      supabaseAdmin.from("Message").select("*").eq("conversationId", conversationId).order("createdAt", { ascending: false }).limit(50),
      supabaseAdmin.from("ConversationSummary").select("*").eq("conversationId", conversationId).order("createdAt", { ascending: false }).limit(1).maybeSingle(),
      supabaseAdmin.from("Memory").select("*").eq("workspaceId", workspaceId).order("updatedAt", { ascending: false }),
      supabaseAdmin.from("UserPreference").select("*").eq("workspaceId", workspaceId).limit(1).maybeSingle(),
      supabaseAdmin.from("User").select("name").eq("id", userId).maybeSingle(),
    ]);

    const recentMessages = (messagesListRes.data || []).reverse();
    const summary = summaryRes.data;
    const memoryRecords: MemoryRecord[] = (allMemoriesRes.data || []) as MemoryRecord[];
    const relevantMemories = retrieveRelevantMemories(content, memoryRecords, 6);
...
// ============================================================
// ============================================================
// TOOL EXECUTOR WITH IDEMPOTENCY TRACKING & TIMEZONE AWARENESS
// ============================================================
...
const toolCallStore = new Map<string, ToolCallRecord>();
...
async function executeTool(
  call: { tool: string; arguments: Record<string, any>; id?: string; toolCallId?: string },
  workspaceId: string,
  userId: string,
  _wsId: string
): Promise<any> {
  ...
  try {
    const { data: pref } = await supabaseAdmin.from("UserPreference").select("timezone").eq("workspaceId", workspaceId).maybeSingle();
    if (pref?.timezone) userTimezone = pref.timezone;
  } catch {}
  ...
  try {
    ...
    if (tool === "calendar.create") {
      ...
      let res = await supabaseAdmin.from("Memory").upsert(
        {
          workspaceId,
          type: memType,
          key,
      ...
      if (res.error && res.error.message?.includes("column")) {
        res = await supabaseAdmin.from("Memory").upsert(
          { workspaceId, type: memType, key, value: content, updatedAt: new Date().toISOString() },
          { onConflict: "workspaceId,type,key" }
        );
      }
      ...
      if (workflow) {
        await supabaseAdmin.from("WorkflowVersion").insert({
...
async function extractAndSaveMemory(
  ...
  try {
    ...
    if (candidate && candidate.shouldRemember) {
      let upsertResult = await supabaseAdmin.from("Memory").upsert(
        {
          workspaceId,
      ...
      if (upsertResult.error && upsertResult.error.message?.includes("column")) {
        upsertResult = await supabaseAdmin.from("Memory").upsert(
          {
            workspaceId,
            type: candidate.memoryType,
            key: candidate.key,
...
app.get("/api/memory", authenticateToken, async (req: AuthenticatedRequest, res) => {
  try {
...
app.post("/api/memory", authenticateToken, async (req: AuthenticatedRequest, res) => {
  ...
  try {
    ...
    if (result.error && result.error.message?.includes("column")) {
      ...
      result = await supabaseAdmin
        .from("Memory")
        .upsert(
          { workspaceId, type: type || "fact", key, value, updatedAt: new Date().toISOString() },
          { onConflict: "workspaceId,type,key" }
        )
        .select()
        .single();
...
app.patch("/api/memory/:id", authenticateToken, async (req: AuthenticatedRequest, res) => {
  ...
  try {
    ...
    if (error) throw error;
    if (!data) return res.status(404).json({ success: false, error: { code: "NOT_FOUND", message: "Memory not found." } });
...
app.delete("/api/memory/all", authenticateToken, async (req: AuthenticatedRequest, res) => {
  try {
    const { error } = await supabaseAdmin
      .from("Memory")
      .delete()
...
app.delete("/api/memory/:id", authenticateToken, async (req: AuthenticatedRequest, res) => {
  try {
    const { error } = await supabaseAdmin
      .from("Memory")
      .delete()
...
app.get("/api/workflows/:id", authenticateToken, async (req: AuthenticatedRequest, res) => {
  const workspaceId = req.user!.workspaceId;
  try {
...
app.post("/api/workflows", authenticateToken, async (req: AuthenticatedRequest, res) => {
...
app.patch("/api/workflows/:id", authenticateToken, async (req: AuthenticatedRequest, res) => {
  ...
  try {
    ...
    if (fetchError || !workflow) return res.status(404).json({ error: "Workflow not found." });
...
app.post("/api/workflows/:id/activate", authenticateToken, async (req: AuthenticatedRequest, res) => {
  const workspaceId = req.user!.workspaceId;
  try {
    const { data: workflow, error: fetchError } = await supabaseAdmin
      .from("Workflow")
    ...
    await supabaseAdmin.from("Workflow").update({ status: "ACTIVE" }).eq("id", workflow.id);

    const nodes = workflow.versions[0].nodes as any[];
    ...
    if (scheduleNode && scheduleNode.data?.cron) {
      ...
      await supabaseAdmin.from("Schedule").delete().eq("workflowId", workflow.id);
...
app.post("/api/workflows/:id/pause", authenticateToken, async (req: AuthenticatedRequest, res) => {
  const workspaceId = req.user!.workspaceId;
  try {
    await supabaseAdmin
      .from("Workflow")
    ...
    await supabaseAdmin.from("Schedule").delete().eq("workflowId", req.params.id);
...
app.post("/api/workflows/:id/test", authenticateToken, async (req: AuthenticatedRequest, res) => {
  const workspaceId = req.user!.workspaceId;
  try {
    const { data: workflow, error: fetchError } = await supabaseAdmin
      .from("Workflow")
...
app.get("/api/executions", authenticateToken, async (req: AuthenticatedRequest, res) => {
  try {
...
app.get("/api/executions/:id", authenticateToken, async (req: AuthenticatedRequest, res) => {
  try {
    const { data: execution, error: execError } = await supabaseAdmin
      .from("Execution")
...
app.get("/api/approvals", authenticateToken, async (req: AuthenticatedRequest, res) => {
  try {
...
app.post("/api/approvals/:id/approve", authenticateToken, async (req: AuthenticatedRequest, res) => {
  const workspaceId = req.user!.workspaceId;
  try {
...
app.post("/api/approvals/:id/reject", authenticateToken, async (req: AuthenticatedRequest, res) => {
  const workspaceId = req.user!.workspaceId;
  try {
    const { data: approval, error: fetchError } = await supabaseAdmin
      .from("Approval")
...
app.get("/api/integrations", authenticateToken, async (req: AuthenticatedRequest, res) => {
  const workspaceId = req.user!.workspaceId;
  try {
 ...
 */
const handleGoogleConnect = (req: AuthenticatedRequest, res: Response) => {
 ...
 */
const handleGoogleStatus = async (req: AuthenticatedRequest, res: Response) => {
  try {
 ...
 */
const handleGoogleDiagnostics = async (req: AuthenticatedRequest, res: Response) => {
  try {
 ...
 */
const handleTestSend = async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    ...
    const result: any = await mail.send(
      recipient,
      subject || "Brain Box Gmail Test",
      body || "This is a test email from Brain Box AI."
    );
  ...
  async (req: AuthenticatedRequest, res) => {
    ...
    try {
      ...
      if (credential) {
        ...
        try {
          const tokenData = JSON.parse(decryptCredential(credential.encryptedData));
          const oauth2Client = getGoogleOAuth2Client();
        ...
        await supabaseAdmin.from("Credential").delete().eq("id", credential.id);
      }
...
app.delete("/api/integrations/google", authenticateToken, async (req: AuthenticatedRequest, res) => {
  const userId = req.user!.userId;
  const workspaceId = req.user!.workspaceId;
  try {
    const { data: credential } = await supabaseAdmin
...
app.post("/api/ai/generate-workflow", authenticateToken, async (req: AuthenticatedRequest, res) => {
...
app.patch("/api/settings", authenticateToken, async (req: AuthenticatedRequest, res) => {
  ...
  if (timezone !== undefined) allowedUpdate.timezone = timezone;
  if (voiceInput !== undefined) allowedUpdate.voiceInput = voiceInput;
  if (voiceAutoSend !== undefined) allowedUpdate.voiceAutoSend = voiceAutoSend;
  if (voiceResponse !== undefined) allowedUpdate.voiceResponse = voiceResponse;
...
app.get("/api/health", async (req, res) => {
  ...
  try {
    const { error } = await supabaseAdmin.from("User").select("count").limit(1);
...
(async () => {
  try {
    const { error } = await (supabaseAdmin.from("User").select("count").limit(1) as any);
    if (error) {
  ...
  const gConfig = validateOAuthConfiguration();
  ...
  if (gConfig.configured) {
    const oauth2Client = getGoogleOAuth2Client();
    const generatedRedirectUri = (oauth2Client as any)._redirectUri || (oauth2Client as any).redirectUri;
